...
write.table(VCV_mat[[1]]$G1_mat,file="Output_files/txt/VM_estimates_N2.txt",sep="\t",quote=FALSE)
write.table(VCV_mat[[2]]$G1_mat,file="Output_files/txt/VM_estimates_PB.txt",sep="\t",quote=FALSE)
...
for(i in 1:1000){
  sampled_variance_N2_null=rbind(sampled_variance_N2_null,c(sum(eigen(matrix(N2_null[(i*6-5):(i*6),],6,6))$values),eigen(matrix(N2_null[(i*6-5):(i*6),],6,6))$values))
  sampled_variance_PB_null=rbind(sampled_variance_PB_null,c(sum(eigen(matrix(PB_null[(i*6-5):(i*6),],6,6))$values),eigen(matrix(PB_null[(i*6-5):(i*6),],6,6))$values))
}



### A new plot with the angle between the g_max of the matrices
...
for(i in 1:1000){
  ...
  all_gmax_angles_N2=c(all_gmax_angles_N2,angle_theta(
    eigen(matrix(VCV_mat[[1]]$VCV_Mat[idx_N2[i],1:36],6,6))$vectors[,1],
    eigen(matrix(VCV_mat[[1]]$VCV_Mat[idx_N2_2[i],1:36],6,6))$vectors[,1]
  ))
  all_gmax_angles_PB=c(all_gmax_angles_PB,angle_theta(
    eigen(matrix(VCV_mat[[2]]$VCV_Mat[idx_PB[i],1:36],6,6))$vectors[,1],
    eigen(matrix(VCV_mat[[2]]$VCV_Mat[idx_PB_2[i],1:36],6,6))$vectors[,1]
  ))
...
all_gmax_angles_N2[all_gmax_angles_N2>90]= 180-all_gmax_angles_N2[all_gmax_angles_N2>90]
all_gmax_angles_PB[all_gmax_angles_PB>90]= 180-all_gmax_angles_PB[all_gmax_angles_PB>90]
...
plot(c(0,0.2),c(
  ...
  sum(eigen(VCV_mat[[2]]$G1_mat)$values)),bg=c("black","black"),pch=21,ylab='Total genetic variance (trace)',xlim=c(-.2,.4),ylim=c(0,1.5),type="n",bty="n",xlab="",xaxt="n")

axis(1,at=c(0,.2),c("N2","PB306"),las=2)
...
plot(eigen(VCV_mat[[1]]$G1_mat)$values~c(1:6),pch=16,xlim=c(.5,6.5),bty="n",ylim=c(0,1),ylab="Genetic variance",type="n",xlab="Eigen vectors",xaxt="n")
#axis(1,at=1:6,c(expression(lambda[gmax]),expression(lambda[2]),expression(lambda[3]),expression(lambda[4]),expression(lambda[5]),expression(lambda[6])))
...
#### Additional panel
pdf(file='plots/Eigen_decomposition_with_NULL_with_angle.pdf',h=7,w=8)

layout(mat=matrix(c(1:3),nrow=1), widths = c(.3,.7,.3))
#par(mfrow=c(1,2),bty="n")

plot(c(0,0.2),c(
  sum(eigen(VCV_mat[[1]]$G1_mat)$values),
  sum(eigen(VCV_mat[[2]]$G1_mat)$values)),bg=c("black","black"),pch=21,ylab='Total genetic variance',xlim=c(-.2,.4),ylim=c(0,1.5),type="n",bty="n",xlab="",xaxt="n")
...
plot(eigen(VCV_mat[[1]]$G1_mat)$values~c(1:6),pch=16,xlim=c(.5,6.5),bty="n",ylim=c(0,1),ylab="Genetic variance",type="n",xlab="Eigen traits",xaxt="n")
#axis(1,at=1:6,c(expression(lambda[gmax]),expression(lambda[2]),expression(lambda[3]),expression(lambda[4]),expression(lambda[5]),expression(lambda[6])))
axis(1,at=1:6,c(expression(m[max]),expression(m[2]),expression(lambda[3]),expression(lambda[4]),expression(lambda[5]),expression(lambda[6])))
for(i in 1:6){
  temp_int <- HPDinterval(mcmc(sampled_variance_N2[,(i+1)]),prob=.95)
  arrows(i,temp_int[1],i,temp_int[2],length=.05,code=3,angle=90)
  temp_int <- HPDinterval(mcmc(sampled_variance_PB[,(i+1)]),prob=.95)
...
temp80 = HPDinterval(as.mcmc(all_gmax_angles),prob=.8)
plot(all_gmax_angles~rep(1,length(all_gmax_angles)),type="n",las=1,bty="n",xlab="",xaxt="n",ylab=
       expression(paste("Angles between ",g[max])),yaxt="s",ylim=c(0,90))
#axis(side=2,at=c(0,22.5,45,67.5,90))
arrows(1,temp95[,1],1,temp95[,2],code=3,angle=90,length=.1)
arrows(1,temp80[,1],1,temp80,length=0,col="firebrick3",lwd=1.5)
...
pdf('plots/angles_btwn_gmax_with_Nulls.pdf',w=3)
plot(all_gmax_angles~rep(1,length(all_gmax_angles)),type="n",las=1,bty="n",xlab="",xaxt="n",ylab=
       expression(paste("Angles between ",g[max])),yaxt="s",xlim=c(0,4),ylim=c(0,90))
#axis(side=2,at=c(0,22.5,45,67.5,90))
temp95 = HPDinterval(as.mcmc(all_gmax_angles))
temp80 = HPDinterval(as.mcmc(all_gmax_angles),prob=.8)
...
temp95 = HPDinterval(as.mcmc(all_gmax_angles_N2))
temp80 = HPDinterval(as.mcmc(all_gmax_angles_N2),prob=.8)
arrows(2,temp95[,1],2,temp95[,2],code=3,angle=90,length=.1)
arrows(2,temp80[,1],2,temp80,length=0,col="firebrick3",lwd=1.5)

temp95 = HPDinterval(as.mcmc(all_gmax_angles_PB))
temp80 = HPDinterval(as.mcmc(all_gmax_angles_PB),prob=.8)
arrows(3,temp95[,1],3,temp95[,2],code=3,angle=90,length=.1)
arrows(3,temp80[,1],3,temp80,length=0,col="firebrick3",lwd=1.5)
...
save(N2_mat ,file="Output_files/VCV_mat_N2.RData")
save(PB_mat ,file="Output_files/VCV_mat_PB.RData")
